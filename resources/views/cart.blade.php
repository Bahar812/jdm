...
@section('content')
    <div class="bg-white">
        ...
        <main class="mx-auto max-w-7xl px-6 py-12 md:py-16">
            ...
            <div class="mb-8">
                <p class="text-xs font-semibold uppercase tracking-[0.35em] text-[color:var(--brand-red)]">Keranjang</p>
                <h1 class="mt-3 font-display text-5xl uppercase text-[color:var(--brand-ink)] md:text-6xl">Checkout Frozen Food</h1>
            </div>
            ...
            @else
                <div class="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
                    <section class="space-y-4 rounded-3xl border border-red-100/70 bg-white p-6 shadow-[0_25px_65px_-40px_rgba(125,12,20,0.45)]">
                        @foreach ($items as $item)
                            <article class="flex flex-col gap-4 rounded-2xl border border-slate-100 p-4 md:flex-row md:items-center">
                                ...
                                <div class="flex-1">
                                    ...
                                    <h2 class="mt-1 font-display text-2xl uppercase text-[color:var(--brand-ink)]">{{ $item['product']->name }}</h2>
                                ...
                                <div class="flex items-center gap-3">
                                    <form action="{{ route('cart.update', $item['product']) }}" method="POST" class="flex items-center gap-2">
                                        ...
                                        <input
                                            ...
                                            value="{{ $item['quantity'] }}"
                                        >
                                        <button class="btn-outline px-3 py-2 text-[10px]" type="submit">Update</button>
                                    </form>
                    ...
                    <section class="rounded-3xl border border-red-100/70 bg-white p-6 shadow-[0_25px_65px_-40px_rgba(125,12,20,0.45)]">
